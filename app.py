...
def generate_quiz_class(class_id):
    ...
    # -----------------------
    # Detect the quiz type
    # -----------------------
    if "mcq" in topic or "multiple choice" in topic:
        prompt = (
        f"Using the following course materials, create 5 multiple-choice questions "
        f"with 4 options each and include the correct answer after each question in the format 'Answer: ...'. "
        f"Keep questions relevant to: {topic}."
    )
    else:
        prompt = (
        f"Using the following course materials, generate 5 practice problems on {topic}. "
        f"For each problem, include the solution after it in the format 'Answer: ...'. "
        f"Do NOT make multiple choice questions. "
        f"Just change the numbers or variables in each question so they are different practice problems."
    )


    system_prompt = (
        "You are an educational assistant creating quizzes based on provided course materials. "
        "Only output plain text — no JSON or markdown. "
        f"Here are the materials:\n\n{full_text}"
    )
